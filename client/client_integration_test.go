...
func TestRecoverFromConflictTransaction(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	client1 := New(ng, chain1.MustId(), remote.NewNetworkPubSub(host))
	defer client1.Stop()
	client2 := New(ng, chain2.MustId(), remote.NewNetworkPubSub(host))
	defer client2.Stop()

	// Two clients send their "first" transaction on a new chaintree around the same time
	resp1, err1 := client1.PlayTransactions(chain1, treeKey, nil, []*chaintree.Transaction{
		{
			Type: "SET_DATA",
			Payload: map[string]string{
				"path": "coolest/client",
	...
	resp2, err2 := client2.PlayTransactions(chain2, treeKey, nil, []*chaintree.Transaction{
		{
			Type: "SET_DATA",
			Payload: map[string]string{
				"path": "lousiest/client",
				"value": "client1",
			},
		},
	})
